...
- **AWS SSO / ECR cleanup - RESOLVED 2026-07-17.** First login attempt authenticated the wrong account/session (`default` -> `137696817339`; `saiyudh_worker` -> `539659138849`, neither is `526404916929`). User then ran `aws sso login --profile skin-lesion-learning-dev` correctly. Deleting the 18 images still failed with `AccessDeniedException: ecr:BatchDeleteImage` - the `SkinLesionVpcLearning` permission set's inline policy (`AllowSkinLesionECRRepositories`, scoped to `arn:aws:ecr:us-east-1:526404916929:repository/skin-lesion-backend-*`) had `ecr:DeleteRepository` but not `ecr:BatchDeleteImage`. User added it and reprovisioned. All 18 images deleted via `aws ecr batch-delete-image` (zero failures), then hit a second, unrelated issue: `terraform destroy` failed with `InvalidGrantException` on its own SSO token refresh even though the AWS CLI's session worked fine - the permission-set reprovision likely invalidated the OIDC refresh token Terraform's Go SDK insists on using, while the CLI kept working off a still-valid cached access token. Declined to work around it by exporting live session credentials into files/env vars. Fixed by a second fresh interactive `aws sso login` (mints a new refresh token). `terraform destroy -var-file=env/staging.tfvars -target=aws_ecr_repository.backend` then succeeded; `terraform state list` confirms empty state. The `dev` state key now has zero tracked resources - the entire forgotten staging environment described below is fully torn down.

518 backend tests pass, ruff/mypy clean, after every round of fixes above (including the consent-route security fixes) - re-verified after each codex round, not just once at the end.
...
| ~~Consent endpoint not idempotent~~ - RESOLVED 2026-07-17 by fixing the root gap it was coupled to. `app/services/consent_service.py` was already a genuinely idempotent state machine, but blocked by `Consent.prediction_id`'s required FK to `predictions.id`, which nothing ever created. Fixed the binary `POST /api/v1/analysis` endpoint (`app/api/v1/router.py`) to require an existing, owned `lesion_id`, upload raw+CAM images to S3 (EXIF-stripped; `app/services/storage_service.py`), and persist a real `Prediction` row - the returned `case_id` is now `Prediction.id`. Codex review (independent pass, not self-reported) found and I fixed: EXIF/GPS metadata was being uploaded unstripped (High - real PHI leak), a failed CAM upload or DB commit could orphan patient images in S3 with no tracking row (High - added compensating deletes), narrow exception handling could 500 instead of 503 on some S3 failures (Low - now also catches `BotoCoreError`), and an audit-log `actor_role` was hardcoded to `"patient"` even when a doctor triggered it (fixed to use `current_user.role`). Also found and fixed a real regression while verifying independently: 4 pre-existing test files used a bare `TestClient(app)` with no lesion/DB fixture and broke once the endpoint started requiring one - added a shared `analysis_upload_client` fixture to `tests/conftest.py`. ~~Multiclass `/api/v1/analysis/multiclass` still does not persist a Prediction~~, ~~`analyze_image` still grants the `doctor` role~~, and ~~the consent endpoint itself is still not built~~ - all three RESOLVED later the same day, 2026-07-17. See the top-of-file entry for the full detail (multiclass got its own `MulticlassPrediction` model/migration, both analysis endpoints are now patient-only, and `POST /api/v1/consent` is built and wired, with a critical cross-patient idempotency-key disclosure bug caught by codex review and fixed before commit). Final test count after all of that: 518 backend tests pass, ruff/mypy clean. | `docs/product/04_PRIVACY_CONSENT_STORAGE_HANDHOLDING.md` |
