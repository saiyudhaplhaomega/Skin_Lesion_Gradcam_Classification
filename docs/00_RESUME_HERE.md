...
- **MINIMAX_API_KEY**: configured (reusing the user's personal `mmx` CLI subscription key - a shared-quota key, not a dedicated production key). Verified live: `run_live_generation_eval` genuinely works now (real MiniMax LLM judging calls succeed; faithfulness=1.0, claim_support_rate=1.0, unsafe_claim_rate=0.0 on a test case). **Found a real, separate bug while verifying**: `answer_relevancy_score` failed its 0.80 threshold (-0.02) - traced to `_embed_texts()` in `admin_market_research_service.py`, which documents a "MiniMax embedding API" priority tier that is never actually implemented in code (only `sentence-transformers`, not installed, then a hash-stub). Setting `MINIMAX_API_KEY` does **not** unblock real embeddings/RAG retrieval quality as previously documented - that requires actually implementing the missing embedding call. `answer_relevancy_score` should be treated as meaningless noise until that's fixed.
...
- ~~No MINIMAX_API_KEY configured~~ - PARTIALLY RESOLVED 2026-07-16. Key configured (user's personal `mmx` CLI key, shared weekly quota - not a dedicated key). `run_live_generation_eval` genuinely works now. Real embeddings for RAG retrieval quality remain unimplemented regardless of this key - see the missing-embedding-tier bug noted above. Fixing that is a real, separate, not-yet-scheduled task.
...
- New decision, not mine to make: whether to implement the missing MiniMax/OpenAI embedding tier in `_embed_texts()` (`admin_market_research_service.py`) so RAG retrieval quality and `answer_relevancy_score` become meaningful instead of stub-noise, and whether to keep reusing the user's personal MiniMax key long-term or provision a dedicated one.
