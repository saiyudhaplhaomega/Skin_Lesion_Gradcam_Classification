...
docs-check:
	...
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/local-dev/10_FRONTEND_SEO_HANDHOLDING.md')) { exit 1 }"
	...
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/03_PROFESSIONAL_FEATURE_SEQUENCE.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/04_PRIVACY_CONSENT_STORAGE_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/05_LESION_BODY_MAPPING_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/06_SAFE_LLM_EXPLANATION_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/07_LANGCHAIN_ADK_OBSERVABILITY_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/10_DOCTOR_ADMIN_REPORTS_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/12_RESEARCH_FAIRNESS_MONITORING_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/08_CUSTOMER_DASHBOARD_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/09_SEO_AND_PUBLIC_PAGES_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/11_LAB_RESULTS_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/16_MOBILE_APP_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/01_BACKEND_DOMAIN_ARCHITECTURE_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/02_DOMAIN_MODEL_AND_API_CONTRACTS_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/17_TRAINING_PIPELINE_MODEL_REGISTRY_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/18_LAB_OCR_EXTRACTION_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/13_LLM_RAG_AGENT_BOUNDARIES_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/15_ADMIN_MARKET_RESEARCH_RAG_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/product/14_ROLE_BASED_EVOLVING_AGENTS_HANDHOLDING.md')) { exit 1 }"
	...
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/06_KUBERNETES_AFTER_DOCKER.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/07_KUBERNETES_LOCAL_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/08_ECR_AND_EKS_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/13_EVENTS_SQS_WORKER_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/12_EVENT_WORKFLOW_AFTER_LOCAL_API.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/14_SECURITY_COMPLIANCE_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/15_OBSERVABILITY_RELIABILITY_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/17_CICD_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/18_LOCAL_TO_STAGING_TO_PRODUCTION_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/19_POWERBI_EMBEDDED_ANALYTICS_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/04_TERRAFORM_PARAMETERS_AND_BOOTSTRAP_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/05_TERRAFORM_STORAGE_SECRETS_AND_ECR_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/10_TERRAFORM_DATABASE_AND_EVENTS_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/11_AURORA_DSQL_STAGING_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/16_TERRAFORM_SECURITY_OBSERVABILITY_HANDHOLDING.md')) { exit 1 }"
	@powershell -NoProfile -Command "if (!(Test-Path 'docs/staging/09_EKS_INGRESS_ALB_CONTROLLER_HANDHOLDING.md')) { exit 1 }"
